const uri = 'https://localhost:44351/api/TaskManager';
let todos = [];
...
function displayEditForm(id) {
    const item = todos.find(item => item.taskId === id);

    document.getElementById('edit-name').value = item.title;
    document.getElementById('edit-id').value = item.taskId;
...
function _displayItems(data) {
    ...
    data.forEach(item => {
        ...
        editButton.setAttribute('onclick', `displayEditForm(${item.taskId})`);

        let deleteButton = button.cloneNode(false);
        deleteButton.innerText = 'Delete';
        deleteButton.setAttribute('onclick', `deleteItem(${item.taskId})`);
